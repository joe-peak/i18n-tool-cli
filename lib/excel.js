...
const getAsyncReadList = (src_dir, lang_dir) => {
  const dataArr = [];
  // ToDo:
  let files = glob.sync(_path.join(src_dir, '**/messages/index.@(js|jsx)'));
  // ToDo:优化 排除node_modules中其他第三方库的影响
  files = files.filter((filePath) => !filePath.includes('node_modules') || filePath.includes('node_modules/supply-componets'));
...
/**
 * classfy the messages by the module
